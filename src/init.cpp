...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "\n" + _("SSL options: (see the Zebra Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (fDaemon)
        fprintf(stdout, "Zebra server starting\n");
                                    ...
                                    BOOST_FOREACH(const CZerocoinEntry& pubCoinItem, listPubCoinInLoop) {
                                        if (pubCoinItem.value == pubCoin) {
                                            ...
                                            BOOST_FOREACH(const CZerocoinEntry& countItemPubcoin, listPubCoinInLoop) {
                                                if (currentId == countItemPubcoin.id && countItemPubcoin.denomination == pubCoinItem.denomination && currentId> 0){
                                                    countExistingItems++;

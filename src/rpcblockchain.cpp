...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            "settxfee <amount ZEA/KB>\n"
            "<amount> is a real and is rounded to the nearest 0.00000001 ZEA per KB");
...
/**
 *
 * 每个比特币网络上的有效交易将被自动的添加到交易池，在这里，它将等待来自矿工的确认。
 * 每当一个矿工将相关交易放入下一个区块，该交易将自动收到第一次确认。
 * 每个节点有自己的mempool，当一个新的有效区块被广播值网络上，每个节点将自动从mempool中移除这些被确认的交易
 * 。因此矿工拿起哪笔交易是具有相当的挑战性(即重要)；
 * 矿工将根据交易费优先选择那些在交易池中未确认的交易。这些交易费分配矿工，
 * 用作他们在比特币网络上解决下一个块的努力的奖金。高交易费的人相对于低交易费能更快的得到确认。
